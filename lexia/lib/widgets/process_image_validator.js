...
- Square PNG image, 1:1 ratio
- Full image canvas with one solid soft background color
- No rounded corners inside the generated image
- No black corners
- No borders, frames, shadows, glow, or vignette
- Subject centered
- Subject size: 60–75% of the image
- No text, letters, numbers, logos, or watermarks
- No realistic or 3D style
- One clear subject only
- Suitable for children aged 5–12

Category guidance:
- animal: one animal only
- food: one food item only
- object: one object only
- nature: one natural element
- action: one person performing the action
- place: simple place/building
- unknown: simplest direct visual representation
...
- Return false if the image contains text, letters, numbers, logos, or watermarks.
...
async function replaceImage(doc, data) {
  ...
  // نفس المسار = يستبدل القديمة تلقائيًا
  ...
  for (const doc of docsToValidate) {
    ...
    try {
      ...
      if (matches) {
        ...
        console.log(`⚠️ ${word} image mismatch. Replacing image...`);
